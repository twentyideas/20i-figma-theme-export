...
const getColorsAndType = () => {
    ...
    const flatList = paintStyles.reduce((all, val) => {
        const name = val.name
            .split(/\s/)
            .map((w, i) => {
...
const getTypographyCSS = () => {
    ...
    const data = textStyles.reduce((all, curr) => {
        ...
        if (!all[name]) {
            all[name] = Object.assign({ elementType, fontFamily: curr.fontName.family, fontWeight: curr.fontName.style === "Bd" ? "bold" : "normal", fontSizeMax: curr.fontSize, fontSizeMin: curr.fontSize }, (curr.lineHeight.unit === "PIXELS" && {

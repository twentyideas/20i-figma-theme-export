...
figma.ui.onmessage = (msg) => {
    ...
    if (msg.type === `styles`) {
        ...
        const keys: Array<keyof ColorMap> = ["primary", "secondary", "accent", "neutral"]
        // Correspond to index of how the color is defined: "primary-<< 1 >>-100"
        const classes: Array<keyof Color> = ["main", "dark", "light", "contrast"]

        const colorMap = flatList.reduce((all: Partial<ColorMap>, curr) => {
            ...
            keys.forEach((key) => {
                if (key === colorKey) {
                    all[key] = {
                        // spread the previous parts of the color
                        ...all[key],
                        [classes[Number(colorClassIdx) - 1]]: {
                            // spread the previous parts so to not overwrite
                            ...all[key]?.[classes[Number(colorClassIdx) - 1]],
                            [opacity]: colorStr,
                        },
                    }
                }
